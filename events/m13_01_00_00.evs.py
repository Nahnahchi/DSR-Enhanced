"""
Tomb of the Giants

linked:


strings:

"""
from soulstruct.darksouls1r.events import *
from soulstruct.darksouls1r.events.instructions import *
from soulstruct.darksouls1r.game_types import *
from ..enums.m13_01_00_00_enums import *
from ..enums.common_enums import CommonFlags


@ContinueOnRest(0)
def Constructor():
    """Event 0"""
    Event_11310992()
    if FlagEnabled(CommonFlags.NitoDead):
        RegisterBonfire(bonfire_flag=11310920, obj=Objects.o0200_0001)
    RegisterBonfire(bonfire_flag=11310992, obj=Objects.o0200_0000)
    RegisterBonfire(bonfire_flag=11310984, obj=Objects.o0200_0002)
    RegisterLadder(start_climbing_flag=11310010, stop_climbing_flag=11310011, obj=Objects.o3043_0000)
    RegisterLadder(start_climbing_flag=11310012, stop_climbing_flag=11310013, obj=Objects.o3044_0000)
    RegisterLadder(start_climbing_flag=11310014, stop_climbing_flag=11310015, obj=Objects.o3140_0000)
    RegisterLadder(start_climbing_flag=11310016, stop_climbing_flag=11310017, obj=Objects.o3140_0001)
    RegisterLadder(start_climbing_flag=11310018, stop_climbing_flag=11310019, obj=Objects.o3141_0000)
    RegisterLadder(start_climbing_flag=11310020, stop_climbing_flag=11310021, obj=Objects.o3142_0000)
    RegisterLadder(start_climbing_flag=11310022, stop_climbing_flag=11310023, obj=Objects.o3143_0001)
    RegisterLadder(start_climbing_flag=11310024, stop_climbing_flag=11310025, obj=Objects.o3144_0000)
    RegisterLadder(start_climbing_flag=11310026, stop_climbing_flag=11310027, obj=Objects.o3143_0000)
    SkipLinesIfClient(2)
    EnableFlag(401)
    DisableFlag(11310000)
    AND_1.Add(Client())
    AND_1.Add(InsideMap(game_map=TOMB_OF_THE_GIANTS))
    SkipLinesIfConditionTrue(2, AND_1)
    DisableObject(Objects.o3961_0000)
    DeleteVFX(VFX.MultiplayerFog1, erase_root_only=False)
    if FlagDisabled(11310810):
        DisableTreasure(obj=Objects.o0500_0024)
        DisableObject(Objects.o0500_0024)
    if FlagEnabled(11310810):
        EnableTreasure(obj=Objects.o0500_0024)
    Event_11310090(0, obj=Objects.o3962_0000, vfx_id=VFX.CheckpointFog1, destination=1312600, destination_1=1312601)
    Event_11315040()
    Event_11315041()
    Event_11315042()
    Event_11310095()
    Event_11315100()
    Event_11310051()
    Event_11310052()
    Event_11310053()
    Event_11310054()
    Event_11310100()
    Event_11315080()
    Event_11315050(0, character=Characters.c2960_0012, entity=Characters.c2960_0012, seconds=0.0)
    Event_11315050(1, character=Characters.c2960_0013, entity=Characters.c2960_0014, seconds=0.0)
    Event_11315050(2, character=Characters.c2960_0014, entity=Characters.c2960_0014, seconds=0.0)
    Event_11315060(0, character=Characters.c2960_0003, flag=51310100, seconds=0.0)
    Event_11315060(1, character=Characters.c2960_0004, flag=51310100, seconds=0.20000000298023224)
    Event_11315060(2, character=Characters.c2960_0005, flag=51310100, seconds=0.8999999761581421)
    Event_11315060(3, character=Characters.c2960_0006, flag=51310100, seconds=1.0)
    Event_11315070(0, character=Characters.c2960_0007, region=1312251, seconds=0.0)
    Event_11315070(1, character=Characters.c2960_0008, region=1312251, seconds=0.20000000298023224)
    Event_11315070(2, character=Characters.c2960_0009, region=1312251, seconds=0.4000000059604645)
    Event_11315070(3, character=Characters.c2960_0010, region=1312251, seconds=0.6000000238418579)
    Event_11315070(4, character=Characters.c2960_0011, region=1312251, seconds=0.800000011920929)
    Event_11310820(0, character=Characters.c2795_0000, item_lot=27903000)
    Event_11310820(1, character=Characters.c3300_0000, item_lot=33005000)
    Event_11310820(2, character=Characters.c2300_ttn1, item_lot=0)
    DisableSoundEvent(sound_id=Sounds.NitoMusic)
    if FlagEnabled(CommonFlags.NitoDead):
        Event_11315392()
        DisableObject(Objects.o3964_0000)
        DeleteVFX(VFX.BossEntranceFog, erase_root_only=False)
    else:
        Event_11315390()
        Event_11315391()
        Event_11315393()
        Event_11315392()
        Event_11310001()
        Event_11315394()
        Event_11315395()
        Event_11315396()
        Event_11315398()
        Event_11314398()
        Event_11315350(0, character=Characters.c2900_0000)
        Event_11315350(1, character=Characters.c2900_0001)
        Event_11315350(2, character=Characters.c2900_0002)
    Event_11315350(3, character=Characters.c2910_0021)
    Event_11315350(4, character=Characters.c2910_0020)
    Event_11315350(5, character=Characters.c2910_0019)
    Event_11315843(0, flag=CommonFlags.NitoDead, line_intersects=1311990, anchor_entity=1312998, target_entity=1312997)
    Event_11315846(0, flag=CommonFlags.NitoDead, obj=Objects.o3964_0000, vfx_id=VFX.BossEntranceFog)


@ContinueOnRest(50)
def Preconstructor():
    """Event 50"""
    HumanityRegistration(Characters.c0000_0007, event_flag=8948)
    Event_11315030()
    Event_11310810()
    HumanityRegistration(Characters.c0000_0002, event_flag=8358)
    SkipLinesIfFlagEnabled(2, 1174)
    SkipLinesIfFlagEnabled(1, 1173)
    DisableCharacter(Characters.c0000_0002)
    Event_11310502(0, character=Characters.c0000_0002, flag=1176)
    Event_11310503(0, character=Characters.c0000_0002, flag=1179)
    Event_11310533(0, character=Characters.c0000_0002, first_flag=1170, last_flag=1181, flag=1177)
    Event_11310534(0, character=Characters.c0000_0002, first_flag=1170, last_flag=1181, flag=1180)
    Event_11310530()
    Event_11310531(0, character=Characters.c0000_0002, first_flag=1170, last_flag=1181, flag=1174)
    Event_11310532(0, character=Characters.c0000_0002, first_flag=1170, last_flag=1181, flag=1175)
    if FlagDisabled(1216):
        DisableCharacter(Characters.c0000_0003)
    SetTeamType(Characters.c0000_0003, TeamType.HostileAlly)
    Event_11310520(1, character=Characters.c0000_0003, first_flag=1210, last_flag=1219, flag=1214)
    if FlagDisabled(1226):
        DisableCharacter(Characters.c0000_0004)
    SetTeamType(Characters.c0000_0004, TeamType.HostileAlly)
    Event_11310520(2, character=Characters.c0000_0004, first_flag=1220, last_flag=1229, flag=1224)
    HumanityRegistration(Characters.c0000_0005, event_flag=8478)
    SkipLinesIfFlagRangeAnyEnabled(1, (1623, 1625))
    DisableCharacter(Characters.c0000_0005)
    Event_11310501(0, character=Characters.c0000_0005, flag=1627)
    Event_11310543(0, character=Characters.c0000_0005, flag=1625)
    Event_11310542(0, character=Characters.c0000_0005, flag=1628)
    Event_11310540(0, character=Characters.c0000_0005, first_flag=1620, last_flag=1631, flag=1623)
    Event_11310541(0, character=Characters.c0000_0005, first_flag=1620, last_flag=1631, flag=1624)
    Event_11310002()


@ContinueOnRest(11310992)
def Event_11310992():
    """Event 11310992"""
    AND_1.Add(FlagEnabled(11027997))
    
    MAIN.Await(AND_1)
    
    AddSpecialEffect(Characters.c5220_0000, 7100)
    AddSpecialEffect(Characters.c5220_0001, 7100)
    AddSpecialEffect(Characters.c2900_0000, 7100)
    AddSpecialEffect(Characters.c2900_0001, 7100)
    AddSpecialEffect(Characters.c2900_0002, 7100)
    AddSpecialEffect(Characters.c2910_0021, 7100)
    AddSpecialEffect(Characters.c2910_0020, 7100)
    AddSpecialEffect(Characters.c2910_0019, 7100)
    AddSpecialEffect(Characters.c0000_0007, 7100)
    AND_2.Add(FlagDisabled(11027997))
    
    MAIN.Await(AND_2)
    
    RemoveSpecialEffect(Characters.c5220_0000, 7100)
    RemoveSpecialEffect(Characters.c5220_0001, 7100)
    RemoveSpecialEffect(Characters.c2900_0000, 7100)
    RemoveSpecialEffect(Characters.c2900_0001, 7100)
    RemoveSpecialEffect(Characters.c2900_0002, 7100)
    RemoveSpecialEffect(Characters.c2910_0021, 7100)
    RemoveSpecialEffect(Characters.c2910_0020, 7100)
    RemoveSpecialEffect(Characters.c2910_0019, 7100)
    RemoveSpecialEffect(Characters.c0000_0007, 7100)
    Restart()


@ContinueOnRest(11310090)
def Event_11310090(_, obj: Object | int, vfx_id: VFXEvent | int, destination: int, destination_1: int):
    """Event 11310090"""
    if ThisEventSlotFlagEnabled():
        DisableObject(obj)
        DeleteVFX(vfx_id, erase_root_only=False)
        End()
    AND_1.Add(ActionButton(
        prompt_text=10010403,
        anchor_entity=destination,
        anchor_type=CoordEntityType.Region,
        model_point=0,
        trigger_attribute=TriggerAttribute.Human | TriggerAttribute.Hollow,
        line_intersects=obj,
    ))
    AND_2.Add(ActionButton(
        prompt_text=10010407,
        anchor_entity=destination_1,
        anchor_type=CoordEntityType.Region,
        model_point=0,
        trigger_attribute=TriggerAttribute.Human | TriggerAttribute.Hollow,
        line_intersects=obj,
    ))
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    
    MAIN.Await(OR_1)
    
    SkipLinesIfFinishedConditionTrue(2, input_condition=AND_2)
    Move(PLAYER, destination=destination, destination_type=CoordEntityType.Region, short_move=True)
    SkipLines(1)
    Move(PLAYER, destination=destination_1, destination_type=CoordEntityType.Region, short_move=True)
    ForceAnimation(PLAYER, 7410)
    DisableObject(obj)
    DeleteVFX(vfx_id)


@RestartOnRest(11315040)
def Event_11315040():
    """Event 11315040"""
    if FlagDisabled(11007999):
        DisableCharacter(Characters.c2910_0022)
        DisableCharacter(Characters.c2910_0023)
        DisableCharacter(1310902)
        DisableCharacter(Characters.c2910_0025)
        DisableCharacter(Characters.c2910_0026)
        DisableCharacter(Characters.c2950_0007)
        DisableCharacter(Characters.c2950_0008)
        DisableCharacter(Characters.c2950_0009)
        DisableCharacter(Characters.c2950_0010)
        DisableCharacter(Characters.c2950_0011)
    OR_1.Add(FlagEnabled(742))
    AND_1.Add(FlagDisabled(11007999))
    AND_1.Add(OR_1)
    if not AND_1:
        return
    EnableFlag(11007999)


@RestartOnRest(11315041)
def Event_11315041():
    """Event 11315041"""
    AND_1.Add(FlagDisabled(742))
    AND_1.Add(FlagEnabled(11007999))
    
    MAIN.Await(AND_1)
    
    DisableFlag(11007999)
    Kill(Characters.c2910_0022)
    Kill(Characters.c2910_0023)
    Kill(1310902)
    Kill(Characters.c2910_0025)
    Kill(Characters.c2910_0026)
    Kill(Characters.c2950_0007)
    Kill(Characters.c2950_0008)
    Kill(Characters.c2950_0009)
    Kill(Characters.c2950_0010)
    Kill(Characters.c2950_0011)


@RestartOnRest(11315042)
def Event_11315042():
    """Event 11315042"""
    if Client():
        return
    AND_1.Add(BlackWorldTendencyComparison(ComparisonType.GreaterThan, value=50))
    AND_1.Add(InsideMap(game_map=TOMB_OF_THE_GIANTS))
    OR_1.Add(AND_1)
    OR_1.Add(FlagEnabled(11310060))
    
    MAIN.Await(OR_1)
    
    WaitFrames(frames=1)
    AND_2.Add(BlackWorldTendencyComparison(ComparisonType.GreaterThan, value=50))
    AND_2.Add(InsideMap(game_map=TOMB_OF_THE_GIANTS))
    OR_2.Add(AND_2)
    OR_2.Add(FlagEnabled(11310060))
    if not OR_2:
        return RESTART
    WaitFrames(frames=1)
    AND_3.Add(BlackWorldTendencyComparison(ComparisonType.GreaterThan, value=50))
    AND_3.Add(InsideMap(game_map=TOMB_OF_THE_GIANTS))
    OR_3.Add(AND_3)
    OR_3.Add(FlagEnabled(11310060))
    if not OR_3:
        return RESTART
    WaitFrames(frames=1)
    AND_4.Add(BlackWorldTendencyComparison(ComparisonType.GreaterThan, value=50))
    AND_4.Add(InsideMap(game_map=TOMB_OF_THE_GIANTS))
    OR_4.Add(AND_4)
    OR_4.Add(FlagEnabled(11310060))
    if not OR_4:
        return RESTART
    WaitFrames(frames=1)
    AND_5.Add(BlackWorldTendencyComparison(ComparisonType.GreaterThan, value=50))
    AND_5.Add(InsideMap(game_map=TOMB_OF_THE_GIANTS))
    OR_5.Add(AND_5)
    OR_5.Add(FlagEnabled(11310060))
    if not OR_5:
        return RESTART
    WaitFrames(frames=1)
    AND_6.Add(BlackWorldTendencyComparison(ComparisonType.GreaterThan, value=50))
    AND_6.Add(InsideMap(game_map=TOMB_OF_THE_GIANTS))
    OR_6.Add(AND_6)
    OR_6.Add(FlagEnabled(11310060))
    if not OR_6:
        return RESTART
    EnableFlag(11310060)
    Wait(600.0)
    AND_7.Add(BlackWorldTendencyComparison(ComparisonType.LessThanOrEqual, value=50))
    AND_7.Add(InsideMap(game_map=TOMB_OF_THE_GIANTS))
    if not AND_7:
        return RESTART
    DisableFlag(11310060)
    EnableFlag(11315045)


@ContinueOnRest(11310095)
def Event_11310095():
    """Event 11310095"""
    if FlagEnabled(11800100):
        EnableFlag(11310096)
        DisableObject(Objects.o3963_0000)
        DeleteVFX(VFX.GoldenFog, erase_root_only=False)
        DisableFlag(401)
        End()
    if FlagEnabled(11310096):
        DisableObject(Objects.o3963_0000)
        DeleteVFX(VFX.GoldenFog, erase_root_only=False)
        End()
    if Client():
        return
    
    MAIN.Await(ActionButton(
        prompt_text=10010410,
        anchor_entity=1312610,
        anchor_type=CoordEntityType.Region,
        model_point=0,
        trigger_attribute=TriggerAttribute.Human | TriggerAttribute.Hollow,
        line_intersects=1311710,
    ))
    
    DisplayStatus(10010630)
    Restart()


@ContinueOnRest(11315390)
def Event_11315390():
    """Event 11315390"""
    AND_1.Add(FlagDisabled(CommonFlags.NitoDead))
    AND_1.Add(CharacterAlive(Characters.c5220_0000))
    AND_1.Add(ActionButton(
        prompt_text=10010403,
        anchor_entity=1312998,
        anchor_type=CoordEntityType.Region,
        trigger_attribute=TriggerAttribute.Human | TriggerAttribute.Hollow,
        boss_version=True,
        line_intersects=1311990,
    ))
    
    MAIN.Await(AND_1)
    
    RotateToFaceEntity(PLAYER, target_entity=1312997)
    ForceAnimation(PLAYER, 7410)
    Restart()


@ContinueOnRest(11315391)
def Event_11315391():
    """Event 11315391"""
    AND_1.Add(FlagDisabled(CommonFlags.NitoDead))
    AND_1.Add(FlagEnabled(11315393))
    AND_1.Add(CharacterWhitePhantom(PLAYER))
    AND_1.Add(ActionButton(
        prompt_text=10010403,
        anchor_entity=1312998,
        anchor_type=CoordEntityType.Region,
        trigger_attribute=TriggerAttribute.All,
        line_intersects=1311990,
    ))
    
    MAIN.Await(AND_1)
    
    RotateToFaceEntity(PLAYER, target_entity=1312997)
    ForceAnimation(PLAYER, 7410)
    Restart()


@ContinueOnRest(11315393)
def Event_11315393():
    """Event 11315393"""
    if ThisEventFlagDisabled():
        AND_1.Add(FlagDisabled(CommonFlags.NitoDead))
        AND_1.Add(FlagEnabled(11315390))
    
        MAIN.Await(AND_1)
    SkipLinesIfClient(1)
    NotifyBossBattleStart()
    ActivateMultiplayerBuffs(Characters.c5220_0000)
    DisableFlag(11310050)


@RestartOnRest(11315392)
def Event_11315392():
    """Event 11315392"""
    if FlagEnabled(CommonFlags.NitoDead):
        DisableCharacter(Characters.c5220_0000)
        Kill(Characters.c5220_0000)
        DisableCharacter(Characters.c5220_0001)
        Kill(Characters.c5220_0001)
        DisableCharacter(Characters.c2900_0000)
        DisableCharacter(Characters.c2900_0001)
        DisableCharacter(Characters.c2900_0002)
        DisableCharacter(Characters.c2910_0021)
        DisableCharacter(Characters.c2910_0020)
        DisableCharacter(Characters.c2910_0019)
        Kill(Characters.c2900_0000)
        Kill(Characters.c2900_0001)
        Kill(Characters.c2900_0002)
        Kill(Characters.c2910_0021)
        Kill(Characters.c2910_0020)
        Kill(Characters.c2910_0019)
        End()
    SkipLinesIfThisEventFlagEnabled(1)
    SkipLinesIfFlagEnabled(1, 11310000)
    DisableCharacter(Characters.c5220_0000)
    DisableAI(Characters.c5220_0000)
    SetStandbyAnimationSettings(Characters.c2900_0000, standby_animation=9001)
    SetStandbyAnimationSettings(Characters.c2900_0001, standby_animation=9001)
    SetStandbyAnimationSettings(Characters.c2900_0002, standby_animation=9001)
    SetStandbyAnimationSettings(Characters.c2910_0021, standby_animation=9001)
    SetStandbyAnimationSettings(Characters.c2910_0020, standby_animation=9001)
    SetStandbyAnimationSettings(Characters.c2910_0019, standby_animation=9001)
    AND_1.Add(Host())
    AND_1.Add(FlagDisabled(11310050))
    AND_1.Add(CharacterInsideRegion(PLAYER, region=1312999))
    
    MAIN.Await(AND_1)
    
    DisableCharacter(Characters.c5220_0001)
    SkipLinesIfFlagEnabled(7, 11310000)
    SkipLinesIfMultiplayer(2)
    PlayCutscene(130100, cutscene_flags=0, player_id=10000)
    SkipLines(1)
    PlayCutscene(130100, cutscene_flags=CutsceneFlags.Unskippable, player_id=10000)
    WaitFrames(frames=1)
    EnableCharacter(Characters.c5220_0000)
    EnableFlag(11310000)
    EnableAI(Characters.c5220_0000)
    EnableBossHealthBar(Characters.c5220_0000, name=5220)
    EnableFlag(11315392)
    DisableNetworkSync()
    SetStandbyAnimationSettings(Characters.c2900_0000)
    ForceAnimation(Characters.c2900_0000, 9061)
    Wait(0.30000001192092896)
    SetStandbyAnimationSettings(Characters.c2900_0001, cancel_animation=9061)
    ForceAnimation(Characters.c2900_0001, 9061)
    Wait(0.5)
    SetStandbyAnimationSettings(Characters.c2900_0002, cancel_animation=9061)
    ForceAnimation(Characters.c2900_0002, 9061)
    Wait(0.20000000298023224)
    SetStandbyAnimationSettings(Characters.c2910_0021, cancel_animation=9061)
    ForceAnimation(Characters.c2910_0021, 9061)
    Wait(0.30000001192092896)
    SetStandbyAnimationSettings(Characters.c2910_0020, cancel_animation=9061)
    ForceAnimation(Characters.c2910_0020, 9061)
    Wait(0.699999988079071)
    SetStandbyAnimationSettings(Characters.c2910_0019, cancel_animation=9061)
    ForceAnimation(Characters.c2910_0019, 9061)


@ContinueOnRest(11310001)
def Event_11310001():
    """Event 11310001"""
    DisableObject(Objects.o0200_0001)
    
    MAIN.Await(CharacterDead(Characters.c5220_0000))
    
    EnableFlag(CommonFlags.NitoDead)
    KillBoss(game_area_param_id=1310800)
    SkipLinesIfClient(1)
    AwardAchievement(achievement_id=35)
    DisableObject(Objects.o3964_0000)
    DeleteVFX(VFX.BossEntranceFog)
    Kill(Characters.c5220_0001)
    CreateTemporaryVFX(vfx_id=90014, anchor_entity=Objects.o0200_0001, anchor_type=CoordEntityType.Object)
    Wait(2.0)
    EnableObject(Objects.o0200_0001)
    RegisterBonfire(bonfire_flag=11310920, obj=Objects.o0200_0001)


@ContinueOnRest(11315394)
def Event_11315394():
    """Event 11315394"""
    DisableNetworkSync()
    AND_1.Add(FlagDisabled(CommonFlags.NitoDead))
    AND_1.Add(FlagEnabled(11315392))
    SkipLinesIfHost(1)
    AND_1.Add(FlagEnabled(11315391))
    
    MAIN.Await(AND_1)
    
    EnableSoundEvent(sound_id=Sounds.NitoMusic)


@ContinueOnRest(11315395)
def Event_11315395():
    """Event 11315395"""
    DisableNetworkSync()
    AND_1.Add(FlagEnabled(CommonFlags.NitoDead))
    AND_1.Add(FlagEnabled(11315394))
    
    MAIN.Await(AND_1)
    
    DisableSoundEvent(sound_id=Sounds.NitoMusic)


@ContinueOnRest(11315396)
def Event_11315396():
    """Event 11315396"""
    MAIN.Await(HealthRatio(Characters.c5220_0000) <= 0.0)
    
    if FlagEnabled(11315370):
        Kill(1310110)
        Kill(1310111)
        Kill(1310112)
        Kill(1310113)
        Kill(1310114)
        Kill(1310115)
        End()
    if FlagEnabled(11315371):
        Kill(1310110, award_souls=True)
        Kill(1310111, award_souls=True)
        Kill(1310112)
        Kill(1310113)
        Kill(1310114)
        Kill(1310115)
        End()
    if FlagEnabled(11315372):
        Kill(1310110, award_souls=True)
        Kill(1310111, award_souls=True)
        Kill(1310112, award_souls=True)
        Kill(1310113, award_souls=True)
        Kill(1310114)
        Kill(1310115)
        End()
    Kill(1310110, award_souls=True)
    Kill(1310111, award_souls=True)
    Kill(1310112, award_souls=True)
    Kill(1310113, award_souls=True)
    Kill(1310114, award_souls=True)
    Kill(1310115, award_souls=True)


@RestartOnRest(11315397)
def Event_11315397():
    """Event 11315397"""
    DisableObject(Objects.o3900_0000)
    DisableObject(Objects.o3901_0000)
    DisableObject(Objects.o3900_0001)
    DisableCharacter(1310110)
    DisableCharacter(1310111)
    DisableCharacter(1310112)
    DisableCharacter(1310113)
    DisableCharacter(1310114)
    DisableCharacter(1310115)
    Event_11315370(
        0,
        flag=11315392,
        part_index=2,
        npc_part_id=0,
        npc_part_id_1=5220,
        obj=5220,
        character=Objects.o3900_0000,
        character_1=1310110,
        animation_id=1310113,
    )
    Event_11315370(
        1,
        flag=11315370,
        part_index=3,
        npc_part_id=0,
        npc_part_id_1=5221,
        obj=5221,
        character=Objects.o3901_0000,
        character_1=1310111,
        animation_id=1310114,
    )
    Event_11315370(
        2,
        flag=11315371,
        part_index=4,
        npc_part_id=0,
        npc_part_id_1=5222,
        obj=5222,
        character=Objects.o3900_0001,
        character_1=1310112,
        animation_id=1310115,
    )
    
    MAIN.Await(CharacterBackreadEnabled(Characters.c5220_0000))
    
    SetDisplayMask(Characters.c5220_0000, bit_index=0, switch_type=OnOffChange.On)


@RestartOnRest(11315398)
def Event_11315398():
    """Event 11315398"""
    DisableNetworkSync()
    AND_1.Add(FlagEnabled(11315390))
    AND_1.Add(CharacterTargeting(targeting_character=Characters.c5220_0000, targeted_character=PLAYER))
    AND_1.Add(CharacterHasTAEEvent(Characters.c5220_0000, tae_event_id=700))
    
    MAIN.Await(AND_1)
    
    EnableFlag(11313398)
    
    MAIN.Await(CharacterHasTAEEvent(Characters.c5220_0000, tae_event_id=710))
    
    EnableFlag(11313399)
    Restart()


@RestartOnRest(11314398)
def Event_11314398():
    """Event 11314398"""
    MAIN.Await(FlagEnabled(11313398))
    
    MoveObjectToCharacter(Objects.o0020_0000, character=PLAYER)
    DisableFlag(11313398)
    
    MAIN.Await(FlagEnabled(11313399))
    
    CreateHazard(
        obj_flag=11315399,
        obj=Objects.o0020_0000,
        model_point=1,
        behavior_param_id=11300,
        target_type=DamageTargetType.Character,
        radius=0.30000001192092896,
        life=0.30000001192092896,
        repetition_time=0.0,
    )
    CreateTemporaryVFX(vfx_id=15224, anchor_entity=Objects.o0020_0000, anchor_type=CoordEntityType.Object)
    DisableFlag(11313399)
    Restart()


@EndOnRest(11315370)
def Event_11315370(
    _,
    flag: Flag | int,
    part_index: short,
    npc_part_id: short,
    npc_part_id_1: int,
    obj: Object | int,
    character: int,
    character_1: int,
    animation_id: int,
):
    """Event 11315370"""
    MAIN.Await(FlagEnabled(flag))
    
    CreateNPCPart(Characters.c5220_0000, npc_part_id=npc_part_id, part_index=part_index, part_health=200)
    AND_1.Add(HealthRatio(Characters.c5220_0000) > 0.0)
    AND_1.Add(CharacterPartHealth(Characters.c5220_0000, npc_part_id=npc_part_id_1) <= 0)
    AND_2.Add(HealthRatio(Characters.c5220_0000) <= 0.0)
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    
    MAIN.Await(OR_1)
    
    EndIfFinishedConditionTrue(input_condition=AND_2)
    SkipLinesIfFlagEnabled(5, 11315370)
    SetCollisionMask(Characters.c5220_0000, bit_index=1, switch_type=OnOffChange.Off)
    SetDisplayMask(Characters.c5220_0000, bit_index=1, switch_type=OnOffChange.On)
    SetDisplayMask(Characters.c5220_0000, bit_index=4, switch_type=OnOffChange.On)
    SetDisplayMask(Characters.c5220_0000, bit_index=7, switch_type=OnOffChange.On)
    SkipLines(10)
    if FlagDisabled(11315371):
        SetCollisionMask(Characters.c5220_0000, bit_index=2, switch_type=OnOffChange.Off)
        SetDisplayMask(Characters.c5220_0000, bit_index=2, switch_type=OnOffChange.On)
        SetDisplayMask(Characters.c5220_0000, bit_index=5, switch_type=OnOffChange.On)
        SetDisplayMask(Characters.c5220_0000, bit_index=8, switch_type=OnOffChange.On)
    else:
        SetCollisionMask(Characters.c5220_0000, bit_index=3, switch_type=OnOffChange.Off)
        SetDisplayMask(Characters.c5220_0000, bit_index=3, switch_type=OnOffChange.On)
        SetDisplayMask(Characters.c5220_0000, bit_index=6, switch_type=OnOffChange.On)
        SetDisplayMask(Characters.c5220_0000, bit_index=9, switch_type=OnOffChange.On)
    EnableObject(obj)
    MoveObjectToCharacter(obj, character=Characters.c5220_0000, model_point=50)
    DestroyObject(obj)
    EnableCharacter(character)
    Move(
        character,
        destination=Characters.c5220_0000,
        destination_type=CoordEntityType.Character,
        model_point=50,
        copy_draw_parent=Characters.c5220_0000,
    )
    ForceAnimation(character, 7000)
    EnableCharacter(character_1)
    Move(
        character_1,
        destination=Characters.c5220_0000,
        destination_type=CoordEntityType.Character,
        model_point=51,
        copy_draw_parent=Characters.c5220_0000,
    )
    ForceAnimation(character_1, 7000)
    ForceAnimation(Characters.c5220_0000, animation_id, wait_for_completion=True)


@RestartOnRest(11315350)
def Event_11315350(_, character: Character | int):
    """Event 11315350"""
    if FlagEnabled(CommonFlags.NitoDead):
        return
    if ThisEventSlotFlagEnabled():
        return
    EnableImmortality(character)
    
    MAIN.Await(HealthRatio(Characters.c5220_0000) <= 0.0)
    
    RemoveSpecialEffect(character, 5451)
    DisableImmortality(character)
    Kill(character, award_souls=True)


@RestartOnRest(11315100)
def Event_11315100():
    """Event 11315100"""
    Event_11315091()
    Event_11315092()
    Event_11315150(0, flag=11315100, vfx_id=VFX.ColoredLight00, flag_1=11315101)
    Event_11315150(1, flag=11315101, vfx_id=VFX.ColoredLight01, flag_1=11315102)
    Event_11315150(2, flag=11315102, vfx_id=VFX.ColoredLight02, flag_1=11315103)
    Event_11315150(3, flag=11315103, vfx_id=VFX.ColoredLight03, flag_1=11315104)
    Event_11315150(4, flag=11315104, vfx_id=VFX.ColoredLight04, flag_1=11315105)
    Event_11315150(5, flag=11315105, vfx_id=VFX.ColoredLight05, flag_1=11315106)
    Event_11315150(6, flag=11315106, vfx_id=VFX.ColoredLight06, flag_1=11315107)
    Event_11315150(7, flag=11315107, vfx_id=VFX.ColoredLight07, flag_1=11315108)
    Event_11315150(8, flag=11315108, vfx_id=VFX.ColoredLight08, flag_1=11315109)
    Event_11315150(9, flag=11315109, vfx_id=VFX.ColoredLight09, flag_1=11315110)
    Event_11315150(10, flag=11315110, vfx_id=VFX.ColoredLight10, flag_1=11315111)
    Event_11315150(11, flag=11315111, vfx_id=VFX.ColoredLight11, flag_1=11315112)
    Event_11315150(12, flag=11315112, vfx_id=VFX.ColoredLight12, flag_1=11315113)
    Event_11315150(13, flag=11315113, vfx_id=VFX.ColoredLight13, flag_1=11315114)
    Event_11315150(14, flag=11315114, vfx_id=VFX.ColoredLight14, flag_1=11315115)
    Event_11315150(15, flag=11315115, vfx_id=VFX.ColoredLight15, flag_1=11315116)
    Event_11315150(16, flag=11315116, vfx_id=VFX.ColoredLight16, flag_1=11315117)
    Event_11315150(17, flag=11315117, vfx_id=VFX.ColoredLight17, flag_1=11315118)
    Event_11315150(18, flag=11315118, vfx_id=VFX.ColoredLight18, flag_1=11315119)
    Event_11315150(19, flag=11315119, vfx_id=VFX.ColoredLight19, flag_1=11315120)
    Event_11315150(20, flag=11315120, vfx_id=VFX.ColoredLight20, flag_1=11315121)
    Event_11315150(21, flag=11315121, vfx_id=VFX.ColoredLight21, flag_1=11315122)
    Event_11315150(22, flag=11315122, vfx_id=VFX.ColoredLight22, flag_1=11315123)
    Event_11315150(23, flag=11315123, vfx_id=VFX.ColoredLight23, flag_1=11315124)
    Event_11315150(24, flag=11315100, vfx_id=VFX.ColoredLight24, flag_1=11315125)
    Event_11315150(25, flag=11315125, vfx_id=VFX.ColoredLight25, flag_1=11315126)
    Event_11315150(26, flag=11315126, vfx_id=VFX.ColoredLight26, flag_1=11315127)
    Event_11315150(27, flag=11315127, vfx_id=VFX.ColoredLight27, flag_1=11315128)
    Event_11315150(28, flag=11315128, vfx_id=VFX.ColoredLight28, flag_1=11315129)
    Event_11315150(29, flag=11315129, vfx_id=VFX.ColoredLight29, flag_1=11315130)
    Event_11315150(30, flag=11315130, vfx_id=VFX.ColoredLight30, flag_1=11315131)
    Event_11315150(31, flag=11315131, vfx_id=VFX.ColoredLight31, flag_1=11315132)
    Event_11315150(32, flag=11315132, vfx_id=VFX.ColoredLight32, flag_1=11315133)
    Event_11315150(33, flag=11315133, vfx_id=VFX.ColoredLight33, flag_1=11315134)
    Event_11315150(34, flag=11315134, vfx_id=VFX.ColoredLight34, flag_1=11315135)
    Event_11315150(35, flag=11315135, vfx_id=VFX.ColoredLight35, flag_1=11315136)
    Event_11315150(36, flag=11315136, vfx_id=VFX.ColoredLight36, flag_1=11315137)
    Event_11315150(37, flag=11315137, vfx_id=VFX.ColoredLight37, flag_1=11315138)
    Event_11315150(38, flag=11315138, vfx_id=VFX.ColoredLight38, flag_1=11315139)
    Event_11315150(39, flag=11315139, vfx_id=VFX.ColoredLight39, flag_1=11315140)
    Event_11315150(40, flag=11315140, vfx_id=VFX.ColoredLight40, flag_1=11315141)
    Event_11315150(41, flag=11315141, vfx_id=VFX.ColoredLight41, flag_1=11315142)
    Event_11315150(42, flag=11315142, vfx_id=VFX.ColoredLight42, flag_1=11315143)
    Event_11315150(43, flag=11315143, vfx_id=VFX.ColoredLight43, flag_1=11315144)
    Event_11315150(44, flag=11315144, vfx_id=VFX.ColoredLight44, flag_1=11315145)
    Event_11315150(45, flag=11315145, vfx_id=VFX.ColoredLight45, flag_1=11315146)
    Event_11315150(46, flag=11315146, vfx_id=VFX.ColoredLight46, flag_1=11315147)
    Event_11315150(47, flag=11315147, vfx_id=VFX.ColoredLight47, flag_1=11315148)
    Event_11315150(48, flag=11315148, vfx_id=VFX.ColoredLight48, flag_1=11315149)


@EndOnRest(11315150)
def Event_11315150(_, flag: Flag | int, vfx_id: VFXEvent | int, flag_1: Flag | int):
    """Event 11315150"""
    DisableNetworkSync()
    if FlagDisabled(11315090):
        DeleteVFX(vfx_id, erase_root_only=False)
        AND_1.Add(FlagEnabled(11315090))
        AND_1.Add(FlagEnabled(flag))
    
        MAIN.Await(AND_1)
    CreateVFX(vfx_id)
    EnableFlag(flag_1)
    
    MAIN.Await(FlagDisabled(11315090))
    
    DeleteVFX(vfx_id)
    DisableFlag(flag_1)
    Restart()


@EndOnRest(11315091)
def Event_11315091():
    """Event 11315091"""
    AND_1.Add(SkullLanternActive())
    AND_1.Add(TimeElapsed(seconds=2.0))
    
    MAIN.Await(AND_1)
    
    EnableFlag(11315090)
    RestartEvent(event_id=11315092)
    Restart()


@EndOnRest(11315092)
def Event_11315092():
    """Event 11315092"""
    DisableNetworkSync()
    
    MAIN.Await(FlagEnabled(11315090))
    
    Wait(3.0)
    DisableFlag(11315090)
    Restart()


@RestartOnRest(11315050)
def Event_11315050(_, character: Character | int, entity: int, seconds: float):
    """Event 11315050"""
    if ThisEventSlotFlagEnabled():
        SetStandbyAnimationSettings(character)
        End()
    SetStandbyAnimationSettings(character, standby_animation=9000)
    
    MAIN.Await(EntityWithinDistance(entity=entity, other_entity=PLAYER, radius=5.0))
    
    Wait(seconds)
    SetStandbyAnimationSettings(character, cancel_animation=9060)
    ReplanAI(character)


@RestartOnRest(11315060)
def Event_11315060(_, character: Character | int, flag: Flag | int, seconds: float):
    """Event 11315060"""
    if FlagEnabled(flag):
        SetStandbyAnimationSettings(character)
        End()
    SetStandbyAnimationSettings(character, standby_animation=9000)
    AND_1.Add(Host())
    SkipLinesIfClient(1)
    AND_1.Add(FlagEnabled(flag))
    
    MAIN.Await(AND_1)
    
    Wait(seconds)
    SetStandbyAnimationSettings(character, cancel_animation=9060)
    ReplanAI(character)


@RestartOnRest(11315070)
def Event_11315070(_, character: Character | int, region: int, seconds: float):
    """Event 11315070"""
    if FlagEnabled(region):
        SetStandbyAnimationSettings(character)
        End()
    SetStandbyAnimationSettings(character, standby_animation=9000)
    
    MAIN.Await(CharacterInsideRegion(PLAYER, region=region))
    
    Wait(seconds)
    SetStandbyAnimationSettings(character, cancel_animation=9060)
    ReplanAI(character)


@RestartOnRest(11315080)
def Event_11315080():
    """Event 11315080"""
    if ThisEventFlagEnabled():
        return
    DisableAI(Characters.c2960_0015)
    AND_1.Add(EntityWithinDistance(entity=Characters.c2960_0015, other_entity=PLAYER, radius=7.0))
    AND_2.Add(Attacked(attacked_entity=Characters.c2960_0015, attacker=PLAYER))
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    
    MAIN.Await(OR_1)
    
    EnableAI(Characters.c2960_0015)
    EndIfFinishedConditionTrue(input_condition=AND_2)
    SetStandbyAnimationSettings(Characters.c2960_0015, standby_animation=9000)
    ForceAnimation(Characters.c2960_0015, 9070, wait_for_completion=True)
    Move(Characters.c2960_0015, destination=1312250, destination_type=CoordEntityType.Region, short_move=True)
    OR_1.Add(EntityWithinDistance(entity=Characters.c2910_0018, other_entity=PLAYER, radius=5.0))
    OR_1.Add(Attacked(attacked_entity=Characters.c2910_0018, attacker=PLAYER))
    
    MAIN.Await(OR_1)
    
    SetStandbyAnimationSettings(Characters.c2960_0015, cancel_animation=9060)


@ContinueOnRest(11310051)
def Event_11310051():
    """Event 11310051"""
    DisableCharacter(Characters.c5220_0001)
    DisableObject(Objects.o3060_0000)
    DisableObjectActivation(Objects.o3060_0000, obj_act_id=-1)
    
    MAIN.Await(FlagEnabled(11310050))
    
    if FlagDisabled(CommonFlags.NitoDead):
        EnableCharacter(Characters.c5220_0001)
    EnableObject(Objects.o3060_0000)
    EnableObjectActivation(Objects.o3060_0000, obj_act_id=-1)
    DisableCharacter(Characters.c2900_0000)
    DisableCharacter(Characters.c2900_0001)
    DisableCharacter(Characters.c2900_0002)
    DisableCharacter(Characters.c2910_0021)
    DisableCharacter(Characters.c2910_0020)
    DisableCharacter(Characters.c2910_0019)
    
    MAIN.Await(FlagDisabled(11310050))
    
    EnableCharacter(Characters.c2900_0000)
    EnableCharacter(Characters.c2900_0001)
    EnableCharacter(Characters.c2900_0002)
    EnableCharacter(Characters.c2910_0021)
    EnableCharacter(Characters.c2910_0020)
    EnableCharacter(Characters.c2910_0019)
    Restart()


@ContinueOnRest(11310052)
def Event_11310052():
    """Event 11310052"""
    MAIN.Await(ObjectActivated(obj_act_id=11315340))
    
    SetStandbyAnimationSettings(PLAYER, standby_animation=7151, death_animation=6082)
    Wait(3.0)
    PlayCutscene(130121, cutscene_flags=0, player_id=10000, move_to_region=1302010, game_map=CATACOMBS)
    PlayCutscene(130021, cutscene_flags=0, player_id=10000)
    WaitFrames(frames=1)
    SetStandbyAnimationSettings(PLAYER)
    EnableObjectActivation(Objects.o3060_0000, obj_act_id=-1)
    Restart()


@ContinueOnRest(11310053)
def Event_11310053():
    """Event 11310053"""
    MAIN.Await(FlagEnabled(11315020))
    
    RotateToFaceEntity(PLAYER, target_entity=Characters.c5220_0001)
    ForceAnimation(PLAYER, 7910, wait_for_completion=True)
    ForceAnimation(PLAYER, 7911, loop=True)
    
    MAIN.Await(FlagDisabled(11315020))
    
    SetStandbyAnimationSettings(PLAYER)
    ForceAnimation(PLAYER, 7912, wait_for_completion=True)
    Restart()


@ContinueOnRest(11310054)
def Event_11310054():
    """Event 11310054"""
    DisableNetworkSync()
    AND_1.Add(Singleplayer())
    AND_1.Add(FlagEnabled(11310050))
    AND_1.Add(EntityWithinDistance(entity=Objects.o3060_0000, other_entity=PLAYER, radius=2.0))
    
    MAIN.Await(AND_1)
    
    Wait(1.0)
    SetStandbyAnimationSettings(PLAYER)
    EnableObjectActivation(Objects.o3060_0000, obj_act_id=-1)
    
    MAIN.Await(EntityBeyondDistance(entity=Objects.o3060_0000, other_entity=PLAYER, radius=3.0))
    
    Restart()


@ContinueOnRest(11310100)
def Event_11310100():
    """Event 11310100"""
    if ThisEventFlagEnabled():
        DisableObject(Objects.o3452_0000)
        End()
    
    MAIN.Await(ObjectDestroyed(Objects.o3452_0000))
    
    End()


@RestartOnRest(11310820)
def Event_11310820(_, character: Character | int, item_lot: int):
    """Event 11310820"""
    if ThisEventSlotFlagEnabled():
        DisableCharacter(character)
        Kill(character)
        End()
    
    MAIN.Await(CharacterDead(character))
    
    if ValueEqual(left=item_lot, right=0):
        return
    OR_7.Add(CharacterHuman(PLAYER))
    OR_7.Add(CharacterHollow(PLAYER))
    if not OR_7:
        return
    AwardItemLot(item_lot, host_only=True)


@ContinueOnRest(11310510)
def Event_11310510(_, character: Character | int, flag: Flag | int):
    """Event 11310510"""
    AND_1.Add(HealthRatio(character) <= 0.8999999761581421)
    AND_1.Add(HealthRatio(character) > 0.0)
    AND_1.Add(Attacked(attacked_entity=character, attacker=PLAYER))
    AND_2.Add(FlagEnabled(flag))
    AND_2.Add(ThisEventSlotFlagEnabled())
    AND_3.Add(FlagEnabled(flag))
    AND_3.Add(ThisEventSlotFlagDisabled())
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    OR_1.Add(AND_3)
    
    MAIN.Await(OR_1)
    
    SkipLinesIfFinishedConditionFalse(2, input_condition=AND_3)
    DisableCharacter(character)
    
    MAIN.Await(FlagEnabled(703))
    
    EnableFlag(flag)
    SetTeamTypeAndExitStandbyAnimation(character, team_type=TeamType.HostileAlly)
    SaveRequest()


@ContinueOnRest(11310520)
def Event_11310520(_, character: Character | int, first_flag: Flag | int, last_flag: Flag | int, flag: Flag | int):
    """Event 11310520"""
    if ThisEventSlotFlagEnabled():
        DropMandatoryTreasure(character)
        End()
    
    MAIN.Await(HealthRatio(character) <= 0.0)
    
    DisableFlagRange((first_flag, last_flag))
    EnableFlag(flag)


@ContinueOnRest(11310501)
def Event_11310501(_, character: Character | int, flag: Flag | int):
    """Event 11310501"""
    AND_1.Add(FlagDisabled(1625))
    AND_1.Add(FlagDisabled(1627))
    AND_1.Add(FlagDisabled(1628))
    OR_7.Add(FlagEnabled(1623))
    OR_7.Add(FlagEnabled(1624))
    AND_1.Add(OR_7)
    AND_1.Add(HealthRatio(character) <= 0.8999999761581421)
    AND_1.Add(HealthRatio(character) > 0.0)
    AND_1.Add(Attacked(attacked_entity=character, attacker=PLAYER))
    AND_1.Add(ThisEventFlagDisabled())
    AND_2.Add(FlagEnabled(flag))
    AND_2.Add(ThisEventFlagEnabled())
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    
    MAIN.Await(OR_1)
    
    EnableFlag(flag)
    SetTeamTypeAndExitStandbyAnimation(character, team_type=TeamType.HostileAlly)
    SaveRequest()
    EnableCharacter(character)


@ContinueOnRest(11310502)
def Event_11310502(_, character: Character | int, flag: Flag | int):
    """Event 11310502"""
    AND_1.Add(FlagDisabled(1176))
    AND_1.Add(FlagDisabled(1179))
    AND_1.Add(FlagEnabled(1173))
    AND_1.Add(HealthRatio(character) <= 0.8999999761581421)
    AND_1.Add(HealthRatio(character) > 0.0)
    AND_1.Add(Attacked(attacked_entity=character, attacker=PLAYER))
    AND_2.Add(FlagEnabled(flag))
    AND_2.Add(ThisEventFlagEnabled())
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    
    MAIN.Await(OR_1)
    
    EnableFlag(flag)
    SetTeamTypeAndExitStandbyAnimation(character, team_type=TeamType.HostileAlly)
    SaveRequest()
    EnableCharacter(character)


@ContinueOnRest(11310503)
def Event_11310503(_, character: Character | int, flag: Flag | int):
    """Event 11310503"""
    AND_1.Add(FlagDisabled(1176))
    AND_1.Add(FlagDisabled(1179))
    AND_1.Add(FlagEnabled(1174))
    AND_1.Add(HealthRatio(character) <= 0.8999999761581421)
    AND_1.Add(HealthRatio(character) > 0.0)
    AND_1.Add(Attacked(attacked_entity=character, attacker=PLAYER))
    AND_2.Add(FlagEnabled(flag))
    AND_2.Add(ThisEventFlagEnabled())
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    
    MAIN.Await(OR_1)
    
    EnableFlag(flag)
    SetTeamTypeAndExitStandbyAnimation(character, team_type=TeamType.HostileAlly)
    SetAIParamID(character, ai_param_id=1)
    SaveRequest()
    EnableCharacter(character)


@ContinueOnRest(11310530)
def Event_11310530():
    """Event 11310530"""
    AND_7.Add(FlagRangeAllDisabled(flag_range=(1176, 1181)))
    AND_7.Add(FlagEnabled(1171))
    AND_7.Add(FlagRangeAllDisabled(flag_range=(1195, 1198)))
    AND_7.Add(FlagEnabled(1202))
    AND_7.Add(FlagRangeAllDisabled(flag_range=(1213, 1215)))
    AND_7.Add(FlagEnabled(1211))
    AND_7.Add(FlagRangeAllDisabled(flag_range=(1223, 1225)))
    AND_7.Add(FlagEnabled(1221))
    AND_1.Add(AND_7)
    AND_1.Add(FlagEnabled(11020600))
    AND_1.Add(ThisEventFlagDisabled())
    AND_2.Add(AND_7)
    AND_2.Add(FlagEnabled(11300005))
    AND_3.Add(AND_7)
    AND_3.Add(ThisEventFlagEnabled())
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    OR_1.Add(AND_3)
    
    MAIN.Await(OR_1)
    
    ClearEventValue(600, bit_count=4)
    EnableFlag(11020693)
    DisableFlagRange((1170, 1189))
    EnableFlag(1173)
    EnableCharacter(Characters.c0000_0002)
    DisableFlagRange((1190, 1209))
    EnableFlag(1192)
    DisableFlagRange((1210, 1219))
    EnableFlag(1216)
    EnableCharacter(Characters.c0000_0003)
    DisableFlagRange((1220, 1229))
    EnableFlag(1226)
    EnableCharacter(Characters.c0000_0004)


@ContinueOnRest(11310531)
def Event_11310531(_, character: Character | int, first_flag: Flag | int, last_flag: Flag | int, flag: Flag | int):
    """Event 11310531"""
    AND_1.Add(FlagDisabled(1176))
    AND_1.Add(FlagDisabled(1179))
    AND_1.Add(FlagEnabled(1173))
    AND_1.Add(FlagEnabled(1214))
    AND_1.Add(FlagEnabled(1224))
    AND_1.Add(CharacterAlive(character))
    
    MAIN.Await(AND_1)
    
    DisableFlagRange((first_flag, last_flag))
    EnableFlag(flag)


@ContinueOnRest(11310532)
def Event_11310532(_, character: Character | int, first_flag: Flag | int, last_flag: Flag | int, flag: Flag | int):
    """Event 11310532"""
    AND_1.Add(Host())
    AND_1.Add(FlagDisabled(1176))
    AND_1.Add(FlagDisabled(1179))
    AND_1.Add(FlagEnabled(1174))
    AND_1.Add(FlagEnabled(11310590))
    AND_1.Add(CharacterBackreadDisabled(character))
    
    MAIN.Await(AND_1)
    
    DisableFlagRange((first_flag, last_flag))
    EnableFlag(flag)
    DisableCharacter(character)
    ClearEventValue(600, bit_count=4)


@ContinueOnRest(11310533)
def Event_11310533(_, character: Character | int, first_flag: Flag | int, last_flag: Flag | int, flag: Flag | int):
    """Event 11310533"""
    AND_1.Add(FlagDisabled(1179))
    OR_7.Add(FlagEnabled(1173))
    OR_7.Add(FlagEnabled(1176))
    AND_1.Add(OR_7)
    AND_1.Add(HealthRatio(character) <= 0.0)
    AND_1.Add(ThisEventFlagDisabled())
    AND_2.Add(FlagEnabled(flag))
    AND_2.Add(ThisEventFlagEnabled())
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    
    MAIN.Await(OR_1)
    
    DisableFlagRange((first_flag, last_flag))
    EnableFlag(flag)
    EndIfFinishedConditionFalse(input_condition=AND_2)
    DropMandatoryTreasure(character)


@ContinueOnRest(11310534)
def Event_11310534(_, character: Character | int, first_flag: Flag | int, last_flag: Flag | int, flag: Flag | int):
    """Event 11310534"""
    AND_1.Add(FlagDisabled(1176))
    OR_7.Add(FlagEnabled(1174))
    OR_7.Add(FlagEnabled(1179))
    AND_1.Add(OR_7)
    AND_1.Add(HealthRatio(character) <= 0.0)
    AND_1.Add(ThisEventFlagDisabled())
    AND_2.Add(FlagEnabled(flag))
    AND_2.Add(ThisEventFlagEnabled())
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    
    MAIN.Await(OR_1)
    
    DisableFlagRange((first_flag, last_flag))
    EnableFlag(flag)
    EndIfFinishedConditionFalse(input_condition=AND_2)
    DropMandatoryTreasure(character)


@ContinueOnRest(11310540)
def Event_11310540(_, character: Character | int, first_flag: Flag | int, last_flag: Flag | int, flag: Flag | int):
    """Event 11310540"""
    AND_1.Add(FlagDisabled(1625))
    AND_1.Add(FlagDisabled(1627))
    AND_1.Add(FlagDisabled(1628))
    AND_2.Add(FlagRangeAnyEnabled(flag_range=(1620, 1621)))
    AND_2.Add(FlagEnabled(CommonFlags.PinwheelDead))
    AND_3.Add(FlagEnabled(1173))
    OR_1.Add(AND_2)
    OR_1.Add(AND_3)
    AND_1.Add(OR_1)
    
    MAIN.Await(AND_1)
    
    DisableFlagRange((first_flag, last_flag))
    EnableFlag(flag)
    EnableCharacter(character)


@ContinueOnRest(11310541)
def Event_11310541(_, character: Character | int, first_flag: Flag | int, last_flag: Flag | int, flag: Flag | int):
    """Event 11310541"""
    AND_1.Add(FlagDisabled(1625))
    AND_1.Add(FlagDisabled(1627))
    AND_1.Add(FlagDisabled(1628))
    AND_1.Add(FlagEnabled(1623))
    AND_1.Add(FlagEnabled(11310002))
    AND_1.Add(CharacterAlive(character))
    
    MAIN.Await(AND_1)
    
    DisableFlagRange((first_flag, last_flag))
    EnableFlag(flag)


@ContinueOnRest(11310542)
def Event_11310542(_, character: Character | int, flag: Flag | int):
    """Event 11310542"""
    AND_1.Add(FlagEnabled(1623))
    AND_1.Add(HealthRatio(character) <= 0.0)
    AND_2.Add(FlagEnabled(1624))
    AND_2.Add(HealthRatio(character) <= 0.0)
    AND_3.Add(FlagEnabled(flag))
    AND_3.Add(ThisEventFlagEnabled())
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    OR_1.Add(AND_3)
    
    MAIN.Await(OR_1)
    
    DisableFlag(1625)
    DisableFlag(1627)
    EnableFlag(flag)
    EndIfFinishedConditionFalse(input_condition=AND_3)
    DropMandatoryTreasure(character)


@ContinueOnRest(11310543)
def Event_11310543(_, character: Character | int, flag: Flag | int):
    """Event 11310543"""
    AND_1.Add(FlagDisabled(1627))
    AND_1.Add(FlagDisabled(1628))
    AND_1.Add(FlagEnabled(1624))
    AND_1.Add(FlagEnabled(11310595))
    AND_1.Add(ThisEventFlagDisabled())
    AND_2.Add(FlagEnabled(flag))
    AND_2.Add(ThisEventFlagEnabled())
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    
    MAIN.Await(OR_1)
    
    EnableFlag(flag)
    SetTeamTypeAndExitStandbyAnimation(character, team_type=TeamType.HostileAlly)
    SaveRequest()
    EnableCharacter(character)


@ContinueOnRest(11315030)
def Event_11315030():
    """Event 11315030"""
    DisableNetworkSync()
    if Client():
        return
    if FlagEnabled(11315031):
        return
    if FlagEnabled(CommonFlags.NitoDead):
        return
    AND_1.Add(Host())
    AND_1.Add(CharacterHuman(PLAYER))
    AND_1.Add(FlagDisabled(11310810))
    if ThisEventFlagDisabled():
        AND_1.Add(CharacterInsideRegion(PLAYER, region=1312001))
    
    MAIN.Await(AND_1)
    
    PlaceSummonSign(
        sign_type=SummonSignType.BlackEyeSign,
        character=Characters.c0000_0007,
        region=1312000,
        summon_flag=11315031,
        dismissal_flag=11315032,
    )
    Wait(20.0)
    Restart()


@ContinueOnRest(11310810)
def Event_11310810():
    """Event 11310810"""
    SkipLinesIfHost(3)
    AND_1.Add(FlagEnabled(11315031))
    AND_1.Add(FlagDisabled(11315032))
    SkipLinesIfConditionTrue(1, AND_1)
    DisableCharacter(Characters.c0000_0007)
    if ThisEventFlagEnabled():
        return
    
    MAIN.Await(CharacterDead(Characters.c0000_0007))
    
    EnableFlag(11310810)


@ContinueOnRest(11310002)
def Event_11310002():
    """Event 11310002"""
    if ThisEventFlagEnabled():
        return
    AND_1.Add(Host())
    AND_1.Add(FlagDisabled(1625))
    AND_1.Add(FlagDisabled(1627))
    AND_1.Add(FlagDisabled(1628))
    AND_1.Add(FlagEnabled(1623))
    AND_1.Add(FlagDisabled(1638))
    AND_1.Add(CharacterInsideRegion(PLAYER, region=1313400))
    AND_2.Add(Host())
    AND_2.Add(FlagDisabled(1625))
    AND_2.Add(FlagDisabled(1627))
    AND_2.Add(FlagDisabled(1628))
    AND_2.Add(FlagEnabled(1623))
    AND_2.Add(FlagEnabled(1638))
    AND_2.Add(CharacterInsideRegion(PLAYER, region=1313400))
    OR_1.Add(AND_1)
    OR_1.Add(AND_2)
    
    MAIN.Await(OR_1)
    
    SkipLinesIfMultiplayer(7)
    SkipLinesIfFinishedConditionTrue(2, input_condition=AND_2)
    PlayCutscene(130111, cutscene_flags=0, player_id=10000, move_to_region=1312120, game_map=TOMB_OF_THE_GIANTS)
    SkipLines(1)
    PlayCutscene(130110, cutscene_flags=0, player_id=10000, move_to_region=1312120, game_map=TOMB_OF_THE_GIANTS)
    WaitFrames(frames=1)
    MoveToEntity(Characters.c0000_0005, destination=1312121, destination_type=CoordEntityType.Region)
    End()
    SkipLinesIfClient(7)
    SkipLinesIfFinishedConditionTrue(2, input_condition=AND_2)
    PlayCutscene(
        130111,
        cutscene_flags=CutsceneFlags.Unskippable,
        player_id=10000,
        move_to_region=1312120,
        game_map=TOMB_OF_THE_GIANTS,
    )
    SkipLines(1)
    PlayCutscene(
        130110,
        cutscene_flags=CutsceneFlags.Unskippable,
        player_id=10000,
        move_to_region=1312120,
        game_map=TOMB_OF_THE_GIANTS,
    )
    WaitFrames(frames=1)
    MoveToEntity(Characters.c0000_0005, destination=1312121, destination_type=CoordEntityType.Region)
    End()
    SkipLinesIfFinishedConditionTrue(2, input_condition=AND_2)
    PlayCutscene(130111, cutscene_flags=CutsceneFlags.Unskippable, player_id=10000)
    SkipLines(1)
    PlayCutscene(130110, cutscene_flags=CutsceneFlags.Unskippable, player_id=10000)
    WaitFrames(frames=1)
    MoveToEntity(Characters.c0000_0005, destination=1312121, destination_type=CoordEntityType.Region)


@ContinueOnRest(11315843)
def Event_11315843(_, flag: Flag | int, line_intersects: int, anchor_entity: int, target_entity: int):
    """Event 11315843"""
    AND_1.Add(Host())
    AND_1.Add(Multiplayer())
    AND_1.Add(FlagEnabled(flag))
    AND_1.Add(ActionButton(
        prompt_text=10010403,
        anchor_entity=anchor_entity,
        anchor_type=CoordEntityType.Region,
        trigger_attribute=TriggerAttribute.Human | TriggerAttribute.Hollow,
        boss_version=True,
        line_intersects=line_intersects,
    ))
    
    MAIN.Await(AND_1)
    
    RotateToFaceEntity(PLAYER, target_entity=target_entity)
    ForceAnimation(PLAYER, 7410, wait_for_completion=True)
    Unknown_2003_47()
    Restart()


@ContinueOnRest(11315846)
def Event_11315846(_, flag: Flag | int, obj: Object | int, vfx_id: VFXEvent | int):
    """Event 11315846"""
    OR_1.Add(Multiplayer())
    OR_1.Add(UnknownPlayerType5())
    AND_1.Add(OR_1)
    AND_1.Add(FlagEnabled(flag))
    
    MAIN.Await(AND_1)
    
    EnableObject(obj)
    CreateVFX(vfx_id)
    AND_3.Add(UnknownPlayerType5())
    AND_2.Add(not AND_3)
    AND_2.Add(Singleplayer())
    
    MAIN.Await(AND_2)
    
    DisableObject(obj)
    DeleteVFX(vfx_id)
    Restart()
